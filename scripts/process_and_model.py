...
# Script has some errors, is missing some text. Figure out how to ensure all text is captured (probably slowing down the autoclicker)
# Also, figure out how to properly give Chizuru context. Because if I fine tune on a one-sided conversation with only Chizuru speaking, it might not work well.
# But if I include responses from other characters...
# Maybe modify make_strings to only take the previous 7 responses and build a conv if it's Chizuru speaking, to ensure that the ai is trained to speak like her
# OR just do what the online example did and filter out everyone who isn't Chizuru. Probably this tbh.
...
# script_df = pd.df(script, columns=['Character','Line'])

# Iterate over this array and append conv to examples
